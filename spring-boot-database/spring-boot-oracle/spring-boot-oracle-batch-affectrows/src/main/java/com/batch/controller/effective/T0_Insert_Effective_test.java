...
public class T0_Insert_Effective_test {
     ...
     * 单次多条insert批量插入(BEGIN END)         单次创建,有无 @Transactional 对效率影响不大, 注意SQL本身具有事务性
     * 单次单条insert批量插入(foreach)           单次创建,有无 @Transactional 对效率影响不大, 注意SQL本身具有事务性
     *--------------------------------------------------------------------------------------------------
     ...
     * 3.batch 模式
     *  不考虑返回行数
     *  <5万条数据: 事务下循环单条操作效率 ≈ JDBC原生batch-prepared {@link com/oracle/vs/jdbc/T2_Run2_JDBC_Insert_Batch}
    ...
    /**
     * ★★★<2000条,推荐写法★★★
     * 单次单条insert批量插入(foreach batch)写法是单条SQL(本身具有事务性),默认配置下,不受 simple/batch 影响
     * 1000 条: 453,45,39
     * 2500 条: 4063,396,81
     * 3000 条: 6409 110 113
