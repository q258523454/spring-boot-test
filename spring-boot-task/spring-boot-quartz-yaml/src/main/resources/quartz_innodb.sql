-- quartz集群就是通过共享一个数据库来实现的，不像redis那样的集群，节点与节点需要通信；
-- quartz集群的节点之间是不需要通信的
-- 你需要在你的quartz.properties文件中设置
-- org.quartz.jobStore.driverDelegateClass = org.quartz.impl.jdbcjobstore.StdJDBCDelegate
-- StdJDBCDelegate说明支持集群，所有的任务信息都会保存到数据库中，可以控制事物，还有就是如果应用服务器关闭或者重启，任务信息都不会丢失，
-- 并且可以恢复因服务器关闭或者重启而导致执行失败的任务
-- 在MySQL数据库中创建以下的表，修改为使用 INNODB 而不是 MYISAM
