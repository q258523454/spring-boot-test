...
public class InitProject {
     ...
     */
    public static String initCronJobName = "init_job";

    /**
     * 启动任务 group
     */
    public static String initCronJobGroup = "init_group";
        ...
        synchronized (log) {
            ...
            for (JobKey jobKey : set) {
                ...
                if (null != jobDetail && InitProject.initCronJobName.equals(jobDetail.getKey().getName()) && InitProject.initCronJobGroup.equals(jobDetail.getKey().getGroup())) {
                    ...
                    // 卸载触发器(删除数据库的前一步操作)
                    scheduler.unscheduleJob(TriggerKey.triggerKey(initJobKey.getName(), initJobKey.getGroup()));
                    // 删除任务(从数据库删除)
            ...
            JobDetail newJobDetail = JobBuilder.newJob(InitCronJob.class)
                    ...
                    // storeDurably() 在没有触发器指向任务的时候，使用 schedule.addJob(quartzjob, true) 将任务保存在队列中了。而后使用 schedule.scheduleJob 触发。
                    // 如果不使用storeDurably, 则在添加 Job 到引擎的时候会抛异常，意思就是该 Job 没有对应的 Trigger
                    .storeDurably()
                    .build();
