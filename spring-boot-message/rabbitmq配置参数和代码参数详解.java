...
put("x-message-ttl",3600*1000);
// 队列中消息总共不能超过1000字节
     args.

put("x-max-length-bytes",1000);
...
reject-publish:当队列消息满了时,

拒绝后续消息(注意是直接reject, 不会再转到死信DLX).
        args.

put("x-overflow","drop-head");
     args.
...
该队列才有可能自动删除(当所有消费者都断开连接, 不管消息是否获取完也会删除)
     new

Queue(TOPIC_QUEUE)默认参数:durable:true,exclusive:false,autoDelete:false
        ----------------------------------------------------
...
持有SSL certificate的key
store的路径
spring.rabbitmq.ssl.key-store-password:
访问key store的密码
spring.rabbitmq.ssl.trust-store:
持有SSL certificates的Trust
store
