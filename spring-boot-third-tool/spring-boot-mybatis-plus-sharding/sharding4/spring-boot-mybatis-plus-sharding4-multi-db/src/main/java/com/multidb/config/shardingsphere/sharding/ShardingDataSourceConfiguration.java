...
@MapperScan(basePackages = "com.multidb.dao.sharding", sqlSessionFactoryRef = "myShardingSqlSessionFactory")
public class ShardingDataSourceConfiguration {

    @Autowired
    private MyShardingSphereProps myShardingProps;

    @Autowired
    private MyShardingRuleConfigurationProps myShardingRule;

    @Autowired
    private MyShardingDataSourceProps myShardingDataSourceProps;
    ...
    /**
     * Sharding 数据源
     * 参考 {@link org.apache.shardingsphere.shardingjdbc.spring.boot.SpringBootConfiguration#shardingDataSource}
     */
    @Bean(name = "myShardingDataSource")
    public DataSource shardingDataSource() throws SQLException {
        return ShardingDataSourceFactory.createDataSource(myShardingDataSourceProps.getDataSourceMap(),
                new ShardingRuleConfigurationYamlSwapper().swap(myShardingRule), myShardingProps.getProps());
    }
    ...
    public PlatformTransactionManager myTransactionManager(@Qualifier("myShardingDataSource") DataSource dynamicDataSource) {
